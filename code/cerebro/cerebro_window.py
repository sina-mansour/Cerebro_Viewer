...
class Cerebro_window(ShowBase):
    def __init__(
        ...
        if not offscreen:
            ...
            self.taskMgr.doMethodLater(
                0.05, self.repeated_checks_task, "repeated_checks_task"
            )
    ...
    def setup_keyboard_and_mouse(self):
        ...
        self.accept(
            "wheel_up", self.update_key_map, ["wheel", (self.key_map["wheel"] + 1)]
        )
        self.accept(
            "wheel_down", self.update_key_map, ["wheel", (self.key_map["wheel"] - 1)]
        )
    ...
    def setup_camera(self):
        ...
        dist = np.sqrt(
            (self.cam_init_x**2) + (self.cam_init_y**2) + (self.cam_init_z**2)
        )
        ...
        # rotation pivots
        self.camera_pivot_node_rl = self.render.attachNewNode(
            "camera-pivot-rl-rotation"
        )
        ...
        self.camera_pivot_node_ud = self.render.attachNewNode(
            "camera-pivot-ud-rotation"
        )
    ...
    def update_camera(
        self, camera_target=None, camera_pos=None, camera_rotation=None, camera_fov=None
    ):
    ...
    def update_task(self, task):
        ...
        if self.key_map["shift"]:
            ...
            if self.key_map["left-click"]:
                rl_position += (
                    self.mouse_x - self.mouse_x_previous
                ) * self.movement_speed
                ud_position += (
                    self.mouse_y - self.mouse_y_previous
                ) * self.movement_speed
        ...
        if (rl_rotation != 0) or (ud_position != 0):
            self.camera_rotation_node_rl = self.render.attachNewNode(
                "camera-pivot-rl-rotation"
            )
            self.camera_rotation_node_rl.setPos(
                self.camera_pivot_node_rl.getPos(self.render)
            )
            self.camera_rotation_node_rl.setHpr(
                self.camera_pivot_node_rl.getHpr(self.render)
            )
        ...
        if (ud_rotation != 0) or (rl_position != 0):
            self.camera_rotation_node_ud = self.render.attachNewNode(
                "camera-pivot-ud-rotation"
            )
            self.camera_rotation_node_ud.setPos(
                self.camera_pivot_node_ud.getPos(self.render)
            )
            self.camera_rotation_node_ud.setHpr(
                self.camera_pivot_node_ud.getHpr(self.render)
            )
    ...
    def create_surface_mesh_node(
        ...
        if vertex_colors is not None:
            vertex_format.add_array(
                core.GeomVertexArrayFormat(
                    "color", 4, core.Geom.NT_uint8, core.Geom.C_color
                )
            )
        # register format
        ...
        vertex_data = core.GeomVertexData(
            "vertex_data", vertex_format, core.Geom.UH_static
        )
    ...
    def create_object_templates(self):
        self.template_sphere_object = self.create_sphere_mesh(
            node_name="template_sphere",
            visualize=False,
            transparent=False,
            reorder_faces=False,
        )
        self.template_cylinder_object = self.create_cylinder_mesh(
            node_name="template_cylinder",
            visualize=False,
            transparent=False,
            reorder_faces=False,
        )
    ...
    def create_multiple_sphere_instances(self, coordinates, radii, colors):
        ...
        for i in range(coordinates.shape[0]):
            sphere_placehoders[i] = self.render.attachNewNode(
                f"sphere-placeholder-{i}#{surface_id}"
            )
    ...
    def create_cylinder_mesh(self, node_name="cylinder", **kwargs):
        cylinder = trimesh.creation.cylinder(height=1.0, radius=1, sections=16)
        return self.create_surface_mesh(
    ...
    def create_multiple_cylinder_instances(self, coordinates, radii, colors):
        ...
        for i in range(coordinates.shape[0]):
            ...
            cylinder_placehoders[i] = core.NodePath(
                f"cylinder-placeholder-{i}#{surface_id}"
            )
            ...
            self.template_cylinder_object["node_path"].instanceTo(
                cylinder_placehoders[i]
            )

            cylinder_placehoder_pivots[i] = self.render.attachNewNode(
                f"cylinder-placeholder-pivot-{i}#{surface_id}"
            )
    ...
    def clear_created_object(self, node_name):
        ...
        if node_type == "surface_mesh":
            ...
            for sphere_placehoder in self.created_objects[node_name][
                "sphere_placehoders"
            ]:
                self.created_objects[node_name]["sphere_placehoders"][
                    sphere_placehoder
                ].removeNode()
        elif node_type == "cylinder_instance":
            for cylinder_placehoder in self.created_objects[node_name][
                "cylinder_placehoders"
            ]:
                self.created_objects[node_name]["cylinder_placehoders"][
                    cylinder_placehoder
                ].removeNode()
            for cylinder_placehoder_pivot in self.created_objects[node_name][
                "cylinder_placehoder_pivots"
            ]:
                self.created_objects[node_name]["cylinder_placehoder_pivots"][
                    cylinder_placehoder_pivot
                ].removeNode()
        else:
    ...
    def offscreen_draw_to_matplotlib_axes(self, ax):
        ...
        self.window_buffer.addRenderTexture(
            bgra_tex, core.GraphicsOutput.RTMCopyRam, core.GraphicsOutput.RTPColor
        )
